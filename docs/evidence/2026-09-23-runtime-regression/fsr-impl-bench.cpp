...
int main(int argc, char** argv) {
    constexpr UINT rw = 1920, rh = 1080, ow = 3840, oh = 2160;
    ...
    ffxCreateContextDescUpscale create{{FFX_API_CREATE_CONTEXT_DESC_TYPE_UPSCALE, &overrideVersion.header},
        FFX_UPSCALE_ENABLE_HIGH_DYNAMIC_RANGE, {rw, rh}, {ow, oh}, nullptr};
    ...
    std::vector<double> apiTimes, completionTimes;
    apiTimes.reserve(measured); completionTimes.reserve(measured);
    for (int frame = 0; frame < warmup + measured; ++frame) {
        ...
        dispatch.enableSharpening = true; dispatch.sharpness = 0.5f;
    ...
    std::printf("BENCH mode=%s provider_id=0x%016llx provider=%s module=%ls dims=%ux%u->%ux%u mask=R8 shader=RCAS@0.5 warm=%d samples=%d api_ms_p50/p90=%.3f/%.3f submit_to_fence_wall_ms_p50/p90=%.3f/%.3f output_mean=%.6f result=PASS\n",

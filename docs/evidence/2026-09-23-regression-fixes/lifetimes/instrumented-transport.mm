...
void unregisterSlot(const ExecutionSlot* slot) noexcept {
    ...
    const auto it = gPending.find(slot->commandBuffer);
    if (it == gPending.end()) return;
    auto& entries = it->second;
    std::erase_if(entries, [slot](const auto& weak) {
...
struct OwnerState;
std::shared_ptr<ExecutionSlot> reserveExecutionSlot(OwnerState& owner, void* commandBuffer,
                                                    ExecutionSlot::Lease initial);

template<class PreparedType>
using ExecutionLeaseFor = std::conditional_t<
    std::is_same_v<PreparedType, std::shared_ptr<const metalfx::PreparedFrame>>,
...
struct OwnerState {
    ...
    std::uint64_t probeReplaySequence = 0;
    ...
    ~OwnerState() {
        std::lock_guard<std::mutex> lock(leases->mutex);
        const auto retainedSlots = leases->slots.size();
        const auto metalBytes = probeDevice
            ? reinterpret_cast<id<MTLDevice>>(probeDevice).currentAllocatedSize : 0;
        std::fprintf(stderr,
                     "YAAGL_LEASE_ALLOCATOR_RESET feature=%llu eval=%llu replays=%llu "
                     "retained_slots=%zu metal_bytes=%llu\n",
                     static_cast<unsigned long long>(featureID),
                     static_cast<unsigned long long>(evaluationID),
                     static_cast<unsigned long long>(probeReplaySequence), retainedSlots,
                     static_cast<unsigned long long>(metalBytes));
        std::fflush(stderr);
        for (const auto& slot : leases->slots) unregisterSlot(slot.get());
    ...
    bool encode(void* command, void* commandBuffer, void* fence) noexcept {
        ...
        try {
            ...
            const std::uint64_t replaySequence = ++probeReplaySequence;
            probeDevice = reinterpret_cast<void*>(
                reinterpret_cast<id<MTL4CommandBuffer>>(commandBuffer).device);
            std::shared_ptr<ExecutionSlot> slot;
            encoded = std::visit([&](const auto& frame) -> bool {
                using T = std::decay_t<decltype(frame)>;
                if constexpr (std::is_same_v<T, std::monostate>) return false;
                else {
                    using Lease = ExecutionLeaseFor<T>;
                    slot = reserveExecutionSlot(*this, commandBuffer, ExecutionSlot::Lease{Lease{}});
                    std::lock_guard<std::mutex> slotLock(slot->mutex);
                    slot->probeFeatureID = featureID;
                    slot->probeEvaluationID = evaluationID;
                    slot->probeReplaySequence = replaySequence;
            ...
            if (!std::visit(
                ...
                return false;
            const auto metalBytes = reinterpret_cast<id<MTLDevice>>(probeDevice).currentAllocatedSize;
            std::fprintf(stderr,
                         "YAAGL_LEASE_ENCODE feature=%llu eval=%llu replay=%llu "
                         "command=%p slot=%p buffer=%p kind=%zu ok=%d metal_bytes=%llu\n",
                         static_cast<unsigned long long>(featureID),
                         static_cast<unsigned long long>(evaluationID),
                         static_cast<unsigned long long>(replaySequence), command, slot.get(),
                         commandBuffer, slot->lease.index(), encoded,
                         static_cast<unsigned long long>(metalBytes));
            std::fflush(stderr);
        } catch (...) {
            return false;
...
std::shared_ptr<ExecutionSlot> reserveExecutionSlot(OwnerState& owner, void* commandBuffer,
                                                    ExecutionSlot::Lease initial) {
    auto slot = std::make_shared<ExecutionSlot>(std::move(initial), commandBuffer, owner.leases);
    owner.leases->retain(slot);
    try {
        std::lock_guard<std::mutex> lock(gPendingLock);
...
void commitRecordedBatch(void* queue, void* selector, const void* const* buffers,
    ...
    if (options && buffers) {
        if (@available(macOS 26.0, *)) {
            try {
                for (std::size_t i = 0; i < count; ++i) {
                    const auto key = const_cast<void*>(buffers[i]);
                    while (true) {
                        std::shared_ptr<ExecutionSlot> slot;
                        {
                            std::lock_guard<std::mutex> lock(gPendingLock);
                            const auto it = gPending.find(key);
                            if (it == gPending.end()) break;
                            auto& entries = it->second;
                            while (!entries.empty() && !slot) {
                        ...
                        @try {
                            [reinterpret_cast<MTL4CommitOptions*>(options)
                                addFeedbackHandler:^(id<MTL4CommitFeedback>) {
                                    std::uint64_t feature = 0, evaluation = 0, replay = 0;
                                    std::size_t kind = 0;
                                    {
                                        std::lock_guard<std::mutex> lock(slot->mutex);
                                        feature = slot->probeFeatureID;
                                        evaluation = slot->probeEvaluationID;
                                        replay = slot->probeReplaySequence;
                                        kind = slot->lease.index();
                                    }
                                    id<MTLDevice> device =
                                        reinterpret_cast<id<MTL4CommandBuffer>>(
                                            slot->commandBuffer).device;
                                    const auto metalBefore = device.currentAllocatedSize;
                                    auto owner = slot->owner.lock();
                                    slot->retire();
                                    std::size_t ownerSlots = 0;
                                    if (owner) {
                                        std::lock_guard<std::mutex> lock(owner->mutex);
                                        ownerSlots = owner->slots.size();
                                    }
                                    const auto metalAfter = device.currentAllocatedSize;
                                    std::fprintf(stderr,
                                                 "YAAGL_LEASE_FEEDBACK feature=%llu eval=%llu "
                                                 "replay=%llu slot=%p buffer=%p kind=%zu "
                                                 "metal_before=%llu metal_after=%llu owner_slots=%zu\n",
                                                 static_cast<unsigned long long>(feature),
                                                 static_cast<unsigned long long>(evaluation),
                                                 static_cast<unsigned long long>(replay), slot.get(),
                                                 slot->commandBuffer, kind,
                                                 static_cast<unsigned long long>(metalBefore),
                                                 static_cast<unsigned long long>(metalAfter), ownerSlots);
                                    std::fflush(stderr);
                                }];
                        } @catch (id) {
                            // The allocator-owned slot remains alive through Reset.
                        }
                    }
                }
            } catch (...) {
                // No GPU work is retired on this path; preserve native submission.
            }
        }
    }
    using Commit = void (*)(id, SEL, const void* const*, NSUInteger, id);
    reinterpret_cast<Commit>(objc_msgSend)(
        reinterpret_cast<id>(queue), reinterpret_cast<SEL>(selector),
        buffers, count, reinterpret_cast<id>(options));
}

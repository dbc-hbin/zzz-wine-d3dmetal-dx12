...
struct Inputs {
    ...
    Inputs(id<MTLDevice> d, NSUInteger w, NSUInteger h, NSUInteger ow, NSUInteger oh)
        : color(texture(d,w,h,MTLPixelFormatRGBA16Float,MTLStorageModePrivate)),
          ...
          reactive(texture(d,w,h,MTLPixelFormatR8Unorm,MTLStorageModePrivate)),
...
struct Runner {
    ...
    void run(const std::shared_ptr<const PreparedFrame>& prepared) {
        ...
        std::printf("GPU_PROBE frame=%u pre_ms=%.4f scaler_ms=%.4f finish_ms=%.4f total_ms=%.4f commit_wall_ms=%.4f\n",frameNumber++,
           ...
           double(entry[3].timestamp-entry[0].timestamp)*1000./freq, wallMs);
